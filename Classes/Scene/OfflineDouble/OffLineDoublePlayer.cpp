...
//overload create() to accept the information of hero1 hero2 and map 
OffLineDoublePlayer* OffLineDoublePlayer::create(char* map, char* aimhero1, char* aimhero2)
{
	...
	//save hero1's and hero2's information and map information into a variable
	pRet->map = map;
...
bool OffLineDoublePlayer::init()
{
	...
	listener->onKeyPressed = CC_CALLBACK_2(OffLineDoublePlayer::onKeyPressed, this);
	listener->onKeyReleased = CC_CALLBACK_2(OffLineDoublePlayer::onKeyReleased, this);

	EventDispatcher* eventDispatcher = Director::getInstance()->getEventDispatcher();
	eventDispatcher->addEventListenerWithSceneGraphPriority(listener, _tileMap);

	//add schedule update
	schedule(schedule_selector(OffLineDoublePlayer::update), 0.01f);
...
void OffLineDoublePlayer::update(float dt)
{
	...
	if (boomflag1 == true && hero1->judgeMap(boomTiledPosition1) && hero1->bubble > 0 && hero1->isAlive == true)
	{
		...
		this->runAction(Sequence::create(delaySetWave, CallFunc::create(CC_CALLBACK_0(OffLineDoublePlayer::addWave, this, boom->position, hero1->power, hero1)), NULL));

		//remove boom in 2.9 second
		auto delayBoom = DelayTime::create(2.9f);
		this->runAction(Sequence::create(delayBoom, CallFunc::create(CC_CALLBACK_0(OffLineDoublePlayer::removeBoom, this, boom)), NULL));
		//change the tile map to make this tile can move
		this->runAction(Sequence::create(delayBoom, CallFunc::create(CC_CALLBACK_0(OffLineDoublePlayer::removeBoomMeta, this, meta, 0, boomTiledPosition1)), NULL));
	}
	...
	if (boomflag2 == true && hero2->judgeMap(boomTiledPosition2) && hero2->bubble > 0 && hero2->isAlive == true)
	{
		...
		//set boom wave in 1.95 secend
		auto delaySetWave = DelayTime::create(1.95f);
		this->runAction(Sequence::create(delaySetWave, CallFunc::create(CC_CALLBACK_0(OffLineDoublePlayer::addWave, this, boom->position, hero2->power, hero2)), NULL));

		//remove boom in 2.9 second
		auto delayBoom = DelayTime::create(2.9f);
		this->runAction(Sequence::create(delayBoom, CallFunc::create(CC_CALLBACK_0(OffLineDoublePlayer::removeBoom, this, boom)), NULL));
		//change the tile map to make this tile can move
		this->runAction(Sequence::create(delayBoom, CallFunc::create(CC_CALLBACK_0(OffLineDoublePlayer::removeBoomMeta, this, meta, 0, boomTiledPosition2)), NULL));
	}
...
//add boom wave
void OffLineDoublePlayer::addWave(Point boomPosition, int Power, Hero* hero)
{
	//set boom crash
	...
	this->runAction(Sequence::create(delayRemoveWave, CallFunc::create(CC_CALLBACK_0(OffLineDoublePlayer::removeWave, this, *it)), NULL));
...
int OffLineDoublePlayer::isCanReach(Point tiledPos)
{
	int tiledGid = meta->getTileGIDAt(tiledPos);
	//judge hard wall
...
Point OffLineDoublePlayer::getTiledPos(Point position)
{
	Point tiledPos;
...
void OffLineDoublePlayer::removeBoom(Boom* boom)
{
...
void OffLineDoublePlayer::giveGifts(Point position)
{
	srand(time(NULL));
	int num = random(1, 8);
	if (num == 1)
...
void OffLineDoublePlayer::gameOver()
{
	...
	//to create a button to select a map and play again
